...
class MirrorListener(listeners.MirrorListeners):
    def __init__(
        self, bot, update, pswd, isTar=False, extract=False, isZip=False, isQbit=False
    ):
    ...
    def onDownloadComplete(self):
        ...
        if self.isTar:
            try:
                ...
                path = (
                    fs_utils.zip(name, m_path) if self.isZip else fs_utils.tar(m_path)
                )
...
def _mirror(bot, update, isTar=False, extract=False, isZip=False, isQbit=False):
    ...
    if (
        ...
        if resp.status_code == 200:
            ...
            sendMessage(
                "ERROR: link got HTTP response:" + resp.status_code, bot, update
            )
            return
